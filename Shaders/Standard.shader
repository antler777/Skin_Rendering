Shader "URP/Standard"
{
    ...
    Properties
    {
        ...
        [KeywordEnum(IndirectLighting, DirectLighting, Both)] _Overlay ("Lighting mode", Float) = 0
        [KeywordEnum(Diffuse_Lambert, Diffuse_Burley ,Diffuse_OrenNayar)] _KEYWORD0 ("Diffuseterm mode", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _KEYWORD0_DIFFUSE_LAMBERT _KEYWORD0_DIFFUSE_BURLEY _KEYWORD0_DIFFUSE_ORENNAYAR            
            // -------------------------------------
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                float Roughness = saturate(SAMPLE_TEXTURE2D(_RoughnessMap,sampler_RoughnessMap,UV).g * _Roughness);
                ...
                half Occlusion = SAMPLE_TEXTURE2D(_OcclusionMap,sampler_OcclusionMap,UV).b;
                ...
                half4 color=(0,0,0,1.0f);
                if (_Overlay==0)
                {
                     color=half4(IndirectLighting,1.0f);
                }
                if (_Overlay==1)
                {
                     color=half4(DirectLighting,1.0f);
                }
                if (_Overlay==2)
                {
                     color = half4(DirectLighting + IndirectLighting,1.0f);
                }
                return color;
            }
            ENDHLSL
