...
void GetCurvature_float(float3 WorldNormal,float3 WorldPos,out float Curvature)
{
    Curvature = 1.0;
    #ifndef SHADERGRAPH_PREVIEW
	float deltaWorldNormal = abs(fwidth(WorldNormal));
	float deltaWorldPosition = abs(fwidth(WorldPos))/0.001;
    Curvature =  saturate(0.3+deltaWorldNormal / deltaWorldPosition*10) ;
    #endif
}	
